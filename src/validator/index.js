...
{
    logger.info( 'Checking props exist......... what do do when they dont?' );

    if( !candidateSchemaObject )
...
const checkTypes = ( validType, value ) =>
{
    ...
    {
        logger.warn( 'actually checking ', value, 'is', typeWeWant )
        switch( typeWeWant )
        {
            case 'schema:Text' : {
                logger.warn( 'testing text' )
...
const checkThisMakesSensePlease = ( referenceSchema, candidateSchemaObject ) =>
{
    ...
    graph.forEach( ( prop, i ) =>
    {
        ...
        if( isValid )
        {
            logger.info( thisProp + ' is not valid' );
        }
...
export default checkThisMakesSensePlease;
